...
def get_twse_data(date_str):
    ...
    # 添加時間戳防止緩存
    url = f'https://www.twse.com.tw/rwd/zh/afterTrading/MI_INDEX?date={date_str}&type=MS&response=json&_={int(time.time()*1000)}'
...
if data:
    table = data['tables'][6]
    with open('每日收盤行情.csv', 'w', newline='', encoding='utf-8-sig') as csvfile:
